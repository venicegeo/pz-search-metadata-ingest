	...
	public @ResponseBody DataResourceContainer createEntryNew(@RequestBody DataResource entry) throws Exception {
		...
		try {
			...
			try{  // in case test or for some other reason null metadata values
				String message = String.format("Error Augmenting JSON Doc with geolocation info, possible null values input, unrecognized EPSG: %d, DataId: %s",
						entry.getSpatialMetadata().getEpsgCode(), entry.getDataId());
	...
	public DataResourceResponse ingestMetadataJob(@RequestBody(required = true) SearchMetadataIngestJob mdingestJob) throws Exception {
		...
		try {
			...
			try {
				...
				try{  // in case test or for some other reason null metadata values
					String message = String.format("Error Augmenting JSON Doc with geolocation info, possible null values input, unrecognized EPSG: %d, DataId: %s",
							dr.getSpatialMetadata().getEpsgCode(), dr.getDataId());
					logger.log(message, PiazzaLogger.WARNING);
				} catch (Exception e2) {
					logger.log(exception.getMessage(), PiazzaLogger.ERROR);
				}
